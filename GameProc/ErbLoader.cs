	...
	internal sealed class ErbLoader
	{
...
#if DEBUG
				output.PrintSystemLine("경과시간2:" + (WinmmTimer.TickCount - starttime).ToString("D4") + "ms:");
#endif
				if (displayReport)
...
#if DEBUG
				output.PrintSystemLine("경과시간3:" + (WinmmTimer.TickCount - starttime).ToString("D4") + "ms:");
#endif
				if (displayReport)
		...
		private void nestCheck(FunctionLabelLine label)
		{
			...
			while (true)
			{
                ...
                {
                    if ((nextLine is GotoLabelLine) && (baseFunc != null))
                    {
						InstructionLine currentBaseFunc = baseFunc;
